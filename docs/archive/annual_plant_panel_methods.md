...
1. Apply documented rules from
   `docs/references/archive/annual_panel/generation_overrides.csv`.
...
- Direct subsidiary observations are already standardized to kilograms by the
  monthly KEPCO pipeline and are summed annually.
...
`docs/references/archive/annual_panel/direct_company_plant_links.csv`. CleanSYS and ENV-INFO
