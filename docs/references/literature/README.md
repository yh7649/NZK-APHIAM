...
| `burnett_2018_pnas_gemm_si.pdf` | Same as above — SI Appendix. | **Actual GEMM fitted parameters (θ, α, μ, ν by cause/age) are in the SI Appendix, not the main text saved above.** The saved file's Methods section gives the functional form (`GEMM(z) = exp{θT(z)}`, `T(z) = log(1+z/α)ω(z)`) but not the fitted numeric parameter values needed to actually compute the function. | still needed | PNAS supplementary information for the DOI above |
