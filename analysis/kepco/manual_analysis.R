...
# ---- Fast thermal EF diagnostics and projections ------------------------------
...
analysis_thermal$plant_unit_id <- paste(
  analysis_thermal$plant_name,
  ifelse(is.na(analysis_thermal$plant_number), "NA", analysis_thermal$plant_number),
  analysis_thermal$energy_type,
  sep = " | "
)
analysis_thermal$energy_type_clean <- ifelse(
  is.na(analysis_thermal$energy_type) | analysis_thermal$energy_type == "",
  "unknown",
  analysis_thermal$energy_type
)
...
for (i in seq_len(nrow(pollutants))) {
  ...
  analysis_thermal[[outlier_var]] <- FALSE

  split_values <- split(analysis_thermal[[ef_var]], analysis_thermal$plant_unit_id)
  ...
  threshold <- q3 + 3 * iqr
  id <- analysis_thermal$plant_unit_id
  analysis_thermal[[outlier_var]] <- !is.na(analysis_thermal[[ef_var]]) &
    n_valid[id] >= 12 &
    iqr[id] > 0 &
    analysis_thermal[[ef_var]] > threshold[id]

  flagged <- analysis_thermal[analysis_thermal[[outlier_var]], c(
  ...
  analysis_thermal[[pollutants$pollutant[[i]]]][analysis_thermal[[outlier_var]]] <- NA_real_
  analysis_thermal[[ef_var]][analysis_thermal[[outlier_var]]] <- NA_real_
}

save_table(outlier_log, "thermal_ef_outliers_removed.csv")
...
summary_rows <- list()
for (energy_type in sort(unique(analysis_thermal$energy_type_clean))) {
  rows <- analysis_thermal$energy_type_clean == energy_type

  for (i in seq_len(nrow(pollutants))) {
    stats <- summarize_vector(analysis_thermal[[pollutants$ef[[i]]]][rows])
    gen_valid <- analysis_thermal$energy_generated_mwh[rows &
      !is.na(analysis_thermal[[pollutants$pollutant[[i]]]]) &
      analysis_thermal$energy_generated_mwh > 0]
    emissions_valid <- analysis_thermal[[pollutants$pollutant[[i]]]][rows &
      !is.na(analysis_thermal[[pollutants$pollutant[[i]]]]) &
      analysis_thermal$energy_generated_mwh > 0]
...
save_table(summary_by_fuel, "thermal_pollutant_summary_by_fuel.csv")

coverage_by_plant_fuel <- aggregate(
  date ~ plant_name + energy_type_clean,
  data = analysis_thermal[
    !is.na(analysis_thermal$nox_kg_per_mwh) |
      !is.na(analysis_thermal$sox_kg_per_mwh) |
      !is.na(analysis_thermal$dust_tsp_kg_per_mwh),
  ],
  FUN = function(x) paste(min(x), max(x), sep = " to ")
)
save_table(coverage_by_plant_fuel, "thermal_ef_coverage_by_plant_fuel.csv")
...
for (i in seq_len(nrow(pollutants))) {
  ...
  output_path <- save_base_png(
    file.path(
      ...
      "selected_plants",
      "ma6",
      paste0("thermal_selected_plants_", pollutants$pollutant[[i]], "_ef_ma6.png")
    )
  )
...
for (i in seq_len(nrow(pollutants))) {
  ...
  save_table(
    plot_data,
    paste0("thermal_fuel_type_monthly_", pollutants$pollutant[[i]], "_ef.csv")
  )
  ...
  output_path <- save_base_png(
    file.path(
      ...
      "fuel_type_averages",
      "ma6",
      paste0("thermal_fuel_type_average_", pollutants$pollutant[[i]], "_ef_ma6.png")
    )
  )
...
fit_break_projection <- function(
  ...
  figure_subdir = file.path("thermal", "projections", "cleaned_monthly")
) {
  ...
  output_path <- save_base_png(
    file.path(
      figure_subdir,
      paste0("thermal_projection_", output_stub, "_", clean_filename(label), "_break_plateau.png")
    )
  )
  plot(
  ...
  save_table(
    projection_table,
    paste0("thermal_projection_", output_stub, "_", clean_filename(label), ".csv")
  )
...
for (i in seq_len(nrow(pollutants))) {
  all_series <- aggregate_ef(analysis_thermal, "date", pollutants$pollutant[[i]])
  ...
  result <- fit_break_projection(
    ...
    figure_subdir = file.path("thermal", "projections", "ma6")
  )
  if (!is.null(result)) {
    projection_summaries[[length(projection_summaries) + 1]] <- result
  }
...
save_table(projection_summary, "thermal_break_projection_summary.csv")

cat("\nFast thermal analysis complete.\n")
